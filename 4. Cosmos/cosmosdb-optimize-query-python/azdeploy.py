...
def _indexing_policy(index_type: str) -> str:
    return (
        '{"indexingMode":"consistent","automatic":true,'
        '"includedPaths":[{"path":"/*"}],'
        '"excludedPaths":[{"path":"/embedding/*"}],'
        f'"vectorIndexes":[{{"path":"/embedding","type":"{index_type}"}}]}}'
    )


CONTAINERS = [
    ("vectors-flat", "flat", "exact nearest neighbor"),
    ("vectors-quantized", "quantizedFlat", "compressed exact search"),
    ("vectors-diskann", "diskANN", "approximate nearest neighbor"),
]
...
def _derived_names(user_object_id: str) -> str:
    ...
    return f"cosmos-index-{user_hash}"
...
def create_cosmosdb_account(account_name: str) -> bool:
    ...
    print()
    print("Use option 2 to create the containers.")
    return True


def create_containers(account_name: str) -> bool:
    print("Creating containers with vector indexing policies...")
    ...
    created = 0
    for name, index_type, description in CONTAINERS:
        exists = az_query(
        ...
        print(f"Creating container '{name}' with {index_type} vector index...")
        if not run_quiet(
            ...
            [
                ...
                "--idx", _indexing_policy(index_type),
                "--vector-embeddings", VECTOR_EMBEDDING_POLICY,
            ],
        ):
            return False
        print(f"Container created: {name} ({description})")
        created += 1

    if created > 0:
        print()
        print("All containers share:")
        print("  - Partition key: /documentId")
        print("  - Vector embedding: /embedding (float32, cosine, 256 dimensions)")
        print()
        print("Index types:")
        print("  - vectors-flat: flat (exact search)")
        print("  - vectors-quantized: quantizedFlat (compressed exact search)")
        print("  - vectors-diskann: diskANN (approximate nearest neighbor)")
...
def check_deployment_status(account_name: str, user_object_id: str) -> bool:
    ...
    for name, _index_type, _description in CONTAINERS:
        exists = az_query(
            ["az", "cosmosdb", "sql", "container", "show",
             "--resource-group", rg, "--account-name", account_name,
             "--database-name", DATABASE_NAME, "--name", name,
        ...
        print(f"  Container {name}: {'Created' if exists else 'Not created'}")
...
def retrieve_connection_info(account_name: str, user_object_id: str) -> bool:
    ...
    print()
    print("Containers: vectors-flat, vectors-quantized, vectors-diskann")
    print()
...
def show_menu(account_name: str) -> None:
    clear_screen()
    print("=====================================================================")
    print("    Azure Cosmos DB Index Optimization Deployment Menu")
    print("=====================================================================")
    ...
    print("2. Create containers (with vector indexing policies)")
...
def main() -> None:
    ...
    while True:
        ...
        if choice == "1":
            ...
            print()
            create_containers(account_name)
            print()
            pause()
        elif choice == "3":
            print()
